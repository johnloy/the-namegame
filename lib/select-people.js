...
function annotateNeighbors (people) {
  ...
  const annotated = map(people, (person, i) => {
    if(i < rowLength) {
      person.neighbors = { down: people[i + rowLength].id }
      person.neighbors.right = people[i + 1].id
      person.neighbors.left = people[i === 0 ? NUM_TO_SHOW - 1 : i - 1].id
    } else {
      person.neighbors = { up: people[i - rowLength].id }
      person.neighbors.right = people[i + 1 === NUM_TO_SHOW ? 0 : i + 1].id
      person.neighbors.left = people[i - 1].id
    }
    return person;
  })
  return annotated
