...
function leastSeen(people) {
  const sorted = sortBy(people, 'seenCount')
  return sorted
}

function females(people) {
  const females = filter(people, 'gender', 'female')
  return females
}

function needsReview(people) {
  const needsReview = filter(people, (p) => p.score < 3)
  return needsReview
}
...
function flagSubject (people, subject) {
...
export default function selectPeople(people) {
  ...
  selected = shuffle(flagSubject(selected, subject));
  selected = addNeighbors(selected)
  return selected
}
